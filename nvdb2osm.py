...
# nvdb2osm.py
# Converts road objects and road networks from NVDB to OSM file format. Supports v4 of api.
# Usage:
...
#server = "https://nvdbapiles.test.atlas.vegvesen.no/"  # ATM - Testproduksjon
server = "https://nvdbapiles.atlas.vegvesen.no/"  # Produksjon
api = "/api/v4/"
...
def tag_highway (segment, lanes, tags, extras):
	...
	if (segment['typeVeg'] in ["Kanalisert veg", "Enkel bilveg"]
				and ref
				and "strekning" in segment['vegsystemreferanse'] and segment['vegsystemreferanse']['strekning']['trafikantgruppe'] == "G"
			or segment['typeVeg'] == "Gang- og sykkelveg"
				and "topologinivå" in segment
				and segment['topologinivå'] == "KJOREBANE"):
	...
	# Tagging for normal highways (cars)
		...
		if ("sideanlegg" in segment['vegsystemreferanse']
				or len(lanes) == 1 and "K" in lanes[0] and ref['vegkategori'] in ["E", "R", "F"] and segment['detaljnivå'] != "Kjørebane"
					and (segment['typeVeg'] != "Enkel bilveg" or "kryssystem" in segment['vegsystemreferanse'])): # Trafikklommer/rasteplasser
			...
			if "nummer" in ref:
				if ref['nummer'] == 162 and ref['vegkategori'] == "R":
					...
					tags['ref'] = "161;Ring 2"
						...
						or ref['nummer'] == 6 and "adresse" in segment and segment['adresse']['navn'] in ["Hjalmar Brantings vei", "Adolf Hedins vei"]):
					tags['ref'] += ";Ring 3"
...
#		tags['bicycle'] = "yes"
	...
	if "adresse" in segment:  # "gate"
		if  segment['typeVeg'] != "Rundkjøring":
			tags['name'] = fix_street_name(segment['adresse']['navn'])
#		if tag_key in tags and tags[tag_key] == "service":  # Upgrade street category if name is present (now done through road object "Adresse")
#			tags[tag_key] = "unclassified"
	...
	if debug:
		...
		if ref:
			...
			if "sideanlegg" in ref:
				extras['SIDEANLEGG'] = "%i-%i" % (ref['sideanlegg']['sideanlegg'], ref['sideanlegg']['sideanleggsdel'])
			if "kryssystem" in ref:
				extras['KRYSSYSTEM'] = "%i-%i" % (ref['kryssystem']['kryssystem'], ref['kryssystem']['kryssdel'])
...
def tag_object (object_id, properties, tags):
	...
	if object_id == "66":  # Avalanche protector
	...
	elif object_id == "913":  # Thoroughfare restriction
		tags['restriction'] = "no_exit"
		if properties['Gjelder for'] == "Lastebil og trekkbil":
			tags['restriction:hgv'] = "no_exit"
		if properties['Merknad'].strip():
			tags['description'] = properties['Merknad'].strip()

		exceptions = {
			'Unntak for buss': 'bus',
			'Unntak for taxi': 'taxi',
			'Unntak for moped': 'moped',
			'Unntak for motorsykkel': 'motorcycle',
			'Unntak for traktor': 'tractor'
		}
		exception_values = []
		for exception, tag in exceptions.items():
			if properties[ exception ] == "Ja":
				exception_values.append(tag)
		if exception_values:
			tags['except'] = ";".join(exception_values)

		periods = []
		if properties['Gjelder hverdager u. lørdag fra kl.']:
			periods.append("Mo-Fr " + properties['Gjelder hverdager u. lørdag fra kl.'] + "-" + properties['Gjelder hverdager u. lørdag til kl.'])
		if properties['Gjelder lørdag fra kl.']:
			periods.append("Sa " + properties['Gjelder lørdag fra kl.'] + "-" + properties['Gjelder lørdag til kl.'])
		if properties['Gjelder søndag fra kl.']:
			periods.append("Su " + properties['Gjelder søndag fra kl.'] + "-" + properties['Gjelder søndag til kl.'])
		if periods:
			tags['restriction:conditional'] = "yes @ " + ", ".join(periods)  # "Gjelder ukedag" not supported
		...
		if "Type" in properties:
			if properties['Type'] in traffic_calmings:
				...
				if properties['Type'] == "Fartshump" and properties['Lengde, langs kjøreretning'] and float(properties['Lengde, langs kjøreretning']) >= 7:
...
#			'Bilsperre': 'gate',
	...
	elif object_id == "854":  # "Kuldeport"
		if "Utgår_Bruksområde" not in properties or properties['Utgår_Bruksområde'] == "Kuldeport":
			tags['barrier'] = "roller_shutter"
			tags['access'] = "yes"
			tags['note'] = "Kuldeport"
		...
		elif properties['Markering av striper'] != "Ikke striper" or properties['Skiltet'] == "Ja":
			tags['crossing'] = "uncontrolled"
		else:
			tags['crossing'] = "unmarked"
		if properties['Markering av striper'] != "Ikke striper":
			tags['crossing:markings'] = "zebra"
		if properties ["Trafikkøy"] == "Ja":
			tags['crossing:island'] = "yes"
		if properties['Hevet'] == "Ja":
			tags['traffic_calming'] = "table"
		if properties['Belysning'] == "Ja":
			tags['lit'] = "yes"
	...
	elif object_id == "43":  # Parking
		tags['amenity'] = "parking"
		if properties['Type'] == "Avskjermet plass":
			tags['parking'] = "surface"
		elif properties['Type'] == "Parkering langs kjørebane":
			tags['parking'] = "street_side"
#		else:
#			tags['parkering'] = "multi-storey"  # Could be "underground"
		if properties['Navn']:
		...
		if properties['Bruksområde'] == "Utfart, tur":
			tags['hiking'] = "yes"
		if properties['Antall parkeringsplasser små kjt.']:
			tags['capacity'] = str(properties['Antall parkeringsplasser små kjt.'])
		if properties['Plasser reservert handikappede']:
			tags['capacity:disabled'] = str(properties['Plasser reservert handikappede'])
		if properties['Antall parkeringsplasser store kjt.']:
			tags['hgv'] = "yes"
...
def update_tags (segment, tags, direction):
	...
	if "name" in tags and "hihghway" in tags or "mainroad" in tags:
	...
	# Maxspeeds may be different for each direction.
		...
		if not ("highway" in segment['tags'] and segment['tags']['highway'] in ["unclassified", "service", "cycleway", "footway", "steps"]):
...
def update_segments_line (parent_sequence_id, tag_start, tag_end, direction, new_tags, new_extras):
	...
	if "tunnel" in new_tags or "bridge" in new_tags:
		for segment_id in parents[ parent_sequence_id ][:]:
			...
			if (("tunnel" in new_tags and "tunnel" in segment['tags'] or "bridge" in new_tags and "bridge" in segment['tags'])
					and (not direction or segment['reverse'] == (direction == "backward"))):
				...
				if (tag_start < segment['parent_start'] + margin and segment['parent_end'] - margin < tag_end
						or segment['parent_start'] + margin < tag_start < segment['parent_end'] - margin
						or segment['parent_start'] + margin < tag_end < segment['parent_end'] - margin):
...
def update_segments_point (parent_sequence_id, tag_position, new_tags, new_extras): 
	...
	for segment_id in parents[ parent_sequence_id ]:
		...
		if segment['parent_start'] <= tag_position <= segment['parent_end']: # and (not side or side == "H" and not segment['reverse'] or side == "V" and segment['reverse']):
			...
			node_tags = segment['geometry'][ node_index ][2]

			# Adjust traffic calming for pedestrian crossing
			if ("highway" in new_tags and new_tags['highway'] == "crossing"
						and "traffic_calming" in node_tags and node_tags['traffic_calming'] == "hump"):
				new_tags['traffic_calming'] = "table"

			node_tags.update(new_tags)

			if debug:
				node_tags.update(new_extras)
...
# Create thoroughfare restrictions

def create_thoroughfare_restriction(start_locations, end_locations, tags, restriction_id):

	# Entry points
	start_nodes = []
	for location in start_locations:
		if "node" in location:
			start_nodes.append(location['node'])
		else:
			for segment_id in parents[ location['veglenkesekvensid'] ][:]:
				segment = segments[ segment_id ]

				if segment['parent_start'] <= location['relativPosisjon'] <= segment['parent_end']:
					if not("oneway" in segment['tags'] and segment['reverse'] == (location['retning'] == "MED")):
						if location['relativPosisjon'] == segment['parent_start']:
							start_nodes.append(segment['start_node'])
						elif location['relativPosisjon'] == segment['parent_end']:
							start_nodes.append(segment['end_node'])
						else:
							new_segment = clip_segment(segment, location['relativPosisjon'])
							start_nodes.append(new_segment['start_node'])

	# Exit points. Note that direction is given as reverse.
	end_nodes = []
	for location in end_locations:
		if "node" in location:
			end_nodes.append(location['node'])
		else:
			for segment_id in parents[ location['veglenkesekvensid'] ][:]:
				segment = segments[ segment_id ]

				if segment['parent_start'] <= location['relativPosisjon'] <= segment['parent_end']:
					if not("oneway" in segment['tags'] and segment['reverse'] == (location['retning'] == "MOT")):
						if location['relativPosisjon'] == segment['parent_start']:
							end_nodes.append(segment['start_node'])
						elif location['relativPosisjon'] == segment['parent_end']:
							end_nodes.append(segment['end_node'])
						else:
							new_segment = clip_segment(segment, location['relativPosisjon'])
							end_nodes.append(new_segment['start_node'])

	# Get unique nodes (duplicates occures at exact start/end of segments)
	start_nodes = list(set(start_nodes))
	end_nodes = list(set(end_nodes))

	# Store restrictions for output
	new_restriction = {
		'from_nodes': start_nodes,
		'to_nodes': end_nodes,
		'tags': tags.copy()
	}

	if not ((not start_nodes) == (not end_nodes)):  # Xor
#		message ("\n  *** Entry or exit point missing from thoroughfare restriction: %s" % restriction_id)
		new_restriction['tags']['fixme'] = "Entry or exit point is missing"

	if start_nodes or end_nodes:
		thoroughfare_restrictions[ restriction_id ] = new_restriction
...
def get_road_object (object_id, **kwargs):
	...
	endpoint_url = server + "vegobjekter" + api + "vegobjekter/"
	object_url = endpoint_url + object_id + "?srid=wgs84&inkluder=metadata,egenskaper,lokasjon"
	if object_id in ["581", "913", "979"]:  # Tunnel, No thoroughfare restriction
		object_url += ",relasjoner"
	if municipality:
...
#			time.sleep(1)
		...
		for road_object in data['objekter']:
			if "stedfestinger" not in road_object['lokasjon']:
				message ("\n  *** 'Stedfestinger' not found: %s\b" % road_object['id'])
				continue
			...
			locations = road_object['lokasjon']['stedfestinger']
			...
			if object_id == "581":
				if "barn" in road_object['relasjoner']:
					for relation in road_object['relasjoner']['barn']:
						if relation['type']['navn'] == "Tunnelløp":
							for segment_id in relation['vegobjekter']:
								tunnels[ segment_id ] = {
									'tags': tags,
									'extras': extras
								}
			...
			# Store thoroughfare exit restriction for output later
			elif object_id == "979" and locations:
				thoroughfare_endings[ road_object['id'] ] = {
					'locations': locations,
					'description': properties['Merknad'].strip()
				}

			# Process thoroughfare restriction
			elif object_id == "913" and locations:
				exit_restriction = road_object['relasjoner']['barn'][0]['vegobjekter'][0]
				if exit_restriction in thoroughfare_endings:
					end_locations = thoroughfare_endings[ exit_restriction ]['locations']
					description = thoroughfare_endings[ exit_restriction ]['description']
				else:
					# Load exit point which may be located in another municiaplity (i.e. not included in loaded data)
					extra_url = endpoint_url + "979/" + str(exit_restriction) + "?srid=wgs84&inkluder=metadata,egenskaper,lokasjon,vegsegmenter,relasjoner"
					extra_data = load_data(extra_url)
					api_calls += 1
					if extra_data['vegsegmenter']:
						# Get description ("underskilt")
						description = ""
						for attribute in extra_data['egenskaper']:
							if attribute['navn'] == "Merknad":
								description = attribute['verdi'].strip()

						thoroughfare_endings[ exit_restriction ] = {
							'locations': extra_data['vegsegmenter'],  # Contains geometry
							'description': description
						}
						end_locations = extra_data['vegsegmenter']
						for end_location in end_locations:
							geometry, geometry_type = process_geometry(end_location['geometri']['wkt'])
							end_location['node'] = create_new_node("", geometry[0][0], set())
							nodes[ end_location['node'] ]['tags']["FIXME"] = "Entry or exit point is in another municipality"

					else:
						message ("\n  *** Road object 979 not found: %s" % exit_restriction)
						end_locations = []
						description = ""

				create_thoroughfare_restriction (locations, end_locations, tags, str(road_object['id']))
				if properties['Retning'] == "Begge":
					reverse_tags = tags.copy()
					if description:
						reverse_tags['description'] = description
					create_thoroughfare_restriction (end_locations, locations, reverse_tags, str(road_object['id']) + "R")  # Reverse

			# Otherwise, update segments or nodes with road object properties
			elif tags:
				if object_id == "47":  # Meeting place
					for i, location in enumerate(locations):
						if location['type'] == "Linje":
							locations[i]['type'] = "Punkt"
							locations[i]['sideposisjon'] = "M"
							locations[i]['relativPosisjon'] = (location['startposisjon'] + location['sluttposisjon']) * 0.5

				elif object_id == "89" and locations[0]['type'] == "Linje":  # Traffic signal
...
#								message ("  *** Equal start and end positions in location - %i\n" % road_object['id'])
						...
						elif location['type'] == "Punkt" and ("sideposisjon" not in location or location['sideposisjon'] == "M" or object_id == "96"):
...
#						message ("  *** Road object sequence %i not found in road network\n" % location['veglenkesekvensid'] )
		...
		if "neste" in data['metadata'] and returned > 0:
...
def process_road_network (segment):
	...
	if (segment['detaljnivå'] != "Vegtrase"
			and ("vegsystemreferanse" not in segment
				or "vegsystem" not in segment['vegsystemreferanse']
				or segment['vegsystemreferanse']['vegsystem']['fase'] != "F")):
...
#			message ("  *** Very short segment %.2fm - %s\n" % (segment['lengde'], segment['referanse']))
		...
		if debug:
			...
			if "superstedfesting" in segment:
				...
				extras['STED_SUPER'] = "%f-%f@%i %s" % (parent['startposisjon'], parent['sluttposisjon'], parent['veglenkesekvensid'], parent['retning'])
				if "sideposisjon" in parent:
					extras['STED_SUPER'] += " " + parent['sideposisjon']
...
def process_road_object (road_object):
	...
	for segment in road_object['vegsegmenter']:
		...
		if "sluttdato" not in segment and "vegsystemreferanse" in segment and "strekning" in segment['vegsystemreferanse']:
			geometry, geometry_type = process_geometry (segment['geometri']['wkt'])
			if len(geometry) > 1:
...
def output_osm(output_filename):
	...
	# Output turn restriction relations
	...
	# Output thoroughfarerestriction relations

	for restriction_id, restriction in iter(thoroughfare_restrictions.items()):
		osm_id -= 1
		osm_relation = ET.Element("relation", id=str(osm_id))
		tag_property (osm_relation, "type", "restriction")
#		tag_property (osm_relation, "restriction", "no_exit")  # Tagged below in tag_property function

		for key, value in iter(restriction['tags'].items()):
			tag_property (osm_relation, key, value)

		if debug:
			tag_property (osm_relation, "ID", restricion_id)  # String

		osm_root.append(osm_relation)

		for node_id in restriction['from_nodes']:
			osm_relation.append(ET.Element("member", type="node", ref=str(nodes[ node_id ]['osmid']), role="from"))  # Note: Only one in wiki
		for node_id in restriction['to_nodes']:
			osm_relation.append(ET.Element("member", type="node", ref=str(nodes[ node_id ]['osmid']), role="to"))
...
#			time.sleep(1)
		...
		data = load_data(url)
#		sys.exit(json.dumps(data, indent=2, ensure_ascii=False) + "\n")
		api_calls += 1
...
#			url += "&fylke=" + ",".join(counties) # All counties
	...
	thoroughfare_endings.clear()
	thoroughfare_restrictions.clear()
...
#		get_road_object ("96", property="(5530=7657)")  # No bicycle nor pedestrian
		...
		get_road_object ("979")  # Thoroughfare restrictions, end - 1st pass
		get_road_object ("913")  # Thoroughfare restrictions, main/start - 2nd pass
...
#	data_status = load_data(server + "status")  # Not supported in api v4
	data_status = load_data(server + "datakatalog/api/v1/versjon")
	message ("Server:         %s\n" % server)
#	message ("API:            v%s, %s\n" % (api_status['nvdbapi-v3'][-1]['version'], api_status['nvdbapi-v3'][-1]['installDate']))
	message ("Data catalogue: v%s, %s\n" % (data_status['versjon'], data_status['dato']))
#	message ("Last DB update: %s\n\n" % data_status['datagrunnlag']['sist_oppdatert'][:10])
	...
	data = load_data(server + "datakatalog/api/v1/vegobjekttyper?inkluder=minimum")
	object_types = {}
	for entry in data:
	...
	if len(sys.argv) > 2:
		if sys.argv[1] == "-vegnett" and len(sys.argv) >= 3:
			...
			url = server + "vegnett" + api + "veglenkesekvenser/segmentert?srid=wgs84" # &kommune=" + municipality
			...
			url = server + "vegnett" + api + "veglenkesekvenser/segmentert?srid=wgs84&vegsystemreferanse=" + sys.argv[2]
			include_objects = False

		elif sys.argv[1] == "-vegobjekt" and len(sys.argv) >= 3 and sys.argv[2].isdigit():
			object_type = sys.argv[2]
			url = server + "vegobjekter" + api + "vegobjekter/" + sys.argv[2] + "?srid=wgs84&inkluder=metadata,egenskaper,geometri,vegsegmenter"  # ,lokasjon
			if object_type in ["581", "913", "979"]:
				url += ",relasjoner"
...
#				longer_ways = False
	...
	tunnels = {}	# Tunnels
	turn_restrictions = {}
	thoroughfare_endings = {}
	thoroughfare_restrictions = {}
